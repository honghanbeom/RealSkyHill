    ...
    void SpawnReferenceObject()
    {
        for (int x = 0; x < gridSize.x; x++)
        {
            ...
            for (int y = 0; y < gridSize.y; y++)
            {
                ...
                referenceObject = Instantiate(referenceObjectPrefab, spawnPosition, Quaternion.identity);
            }
        }
    }
    ...
    void SpawnObjects()
    {
        for (int x = 0; x < gridSize.x; x++)
        {
            ...
            for (int y = 0; y < gridSize.y; y++)
            {
                ...
                Instantiate(objectPrefab, rightSpawnPosition, Quaternion.identity);

                // 왼쪽에 GameObject 생성
                Vector3 leftSpawnPosition = spawnPosition + Vector3.left * spacingLeft;
                Instantiate(objectPrefab, leftSpawnPosition, Quaternion.identity);
